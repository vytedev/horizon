...
class ProductFormComponent extends Component {
  ...
  #onVariantUpdate = (event) => {
    ...
    if (event.detail.resource == null || event.detail.resource.available == false) {
      ...
      this.refs.acceleratedCheckoutButtonContainer?.setAttribute('data-disabled', 'true');
      // Disable the payment button and show "Sold Out"
      const paymentButton = this.refs.acceleratedCheckoutButtonContainer?.querySelector('button[data-shopify-payment-button], shopify-payment-button button, shopify-accelerated-checkout button');
      if (paymentButton) {
      ...
      this.refs.acceleratedCheckoutButtonContainer?.removeAttribute('data-disabled');
      // Enable the payment button and restore "Buy with Shop" text
      const paymentButton = this.refs.acceleratedCheckoutButtonContainer?.querySelector('button[data-shopify-payment-button], shopify-payment-button button, shopify-accelerated-checkout button');
      if (paymentButton) {
        paymentButton.disabled = false;
        // Reset customized flag to allow re-customization
        paymentButton.dataset.customized = 'false';
        // The existing customizeButton intervals will handle re-customization
      }
    }
